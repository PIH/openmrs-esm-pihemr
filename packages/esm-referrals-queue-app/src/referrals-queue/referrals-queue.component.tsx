...
export default function ReferralsQueue(props: ReferralsQueueProps) {
  ...
  const filteredReferrals = referrals
    .filter(
      (r) => !referralTypeFilter || referralTypeFilter == t('any', 'Any') || r.referral_type == referralTypeFilter,
    )
    .filter((r) => !statusFilter || statusFilter == t('any', 'Any') || r.fulfillment_status == statusFilter)
    .filter((r) => matchQuery(r, ptQuery));

  const referralTypes = [t('any', 'Any'), ...new Set(referrals.map((r) => r.referral_type).filter((r) => r != null))];

  const statuses = [t('any', 'Any'), ...new Set(referrals.map((r) => r.fulfillment_status).filter((r) => r != null))];
        ...
        <div className={styles.controlsContainer}>
          ...
          <div className={styles.inputContainer}>
            <div style={{ width: 400 }}>
              ...
              <Search id="query-input" labelText="" onChange={(e) => setPtQuery(e.target.value)} />
            </div>
          </div>
          <div className={styles.inputContainer}>
            <div style={{ width: 400 }}>
...
function matchQuery(referral: Referral, query: string): boolean {
  return (
    !query ||
    prepareQuery(query).every((regexp) => regexp.test(removeDiacritics(referral.patient_name))) ||
