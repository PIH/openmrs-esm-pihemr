...
export function NextVisitTile({ patientUuid }: nextVisitTileProps) {
  const { nextVisitDate, conceptName, isError, isLoading, isValidating } = useNextVisit(patientUuid);
  ...
  return (
    <Tile light>
      <ValueTile label={conceptName ?? 'Next visit Date'}>
        ...
        {isError ? 'Error' : null}
        {!isLoading && nextVisitDate ? formatDate(nextVisitDate, { time: false }) ?? '—' : null}
      </ValueTile>
    </Tile>
  );
}
