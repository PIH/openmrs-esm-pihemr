import { openmrsFetch, fhirBaseUrl, parseDate, useConfig } from '@openmrs/esm-framework';
import useSWR from 'swr';
import { type PatientStatusWidgetConfig } from './config-schema';
...
export function useDiagnosis(patientUuid: string) {
  ...
  const { data, error, isValidating } = useSWR<{ data: PatientProgramStateResponse }, Error>(apiUrl, openmrsFetch);
  ...
  const oncologyEnrollment = data?.data.results.filter(
    (enrollment) => !enrollment.voided && enrollment.program.name == config.oncologyProgramName,
  )[0];
  if (oncologyEnrollment) {
    const diagnosisWorkflow = oncologyEnrollment.program.allWorkflows.filter(
      (workflow) => !workflow.retired && workflow.concept.uuid == config.diagnosisWorkflowConceptUuid,
    )[0];
...
function useObs(patientUuid: string, conceptUuid: string) {
  ...
  const { data, error, isValidating } = useSWR<{ data: RESTObsResponse }, Error>(apiUrl, openmrsFetch);

  const obsSelected = data?.data.results.filter((observation) => !observation.voided)[0];

  const obsValue = typeof obsSelected?.value == 'string' ? obsSelected?.value : obsSelected?.value.display;
  return {
    obsValue,
    isError: error,
    isLoading: !data && !error,
...
export function useNextVisit(patientUuid: string) {
  ...
  const { data, error, isValidating } = useSWR<{ data: FHIRObsResponse }, Error>(apiUrl, openmrsFetch);

  const chosenObs = data?.data.entry ? chooseNextVisitDateObs(data.data.entry) : null;

  return {
    conceptName: chosenObs?.resource.code.text,
    nextVisitDate: chosenObs ? parseDate(chosenObs.resource.valueDateTime) : null,
    isError: error,
    isLoading: !data && !error,
    isValidating,
  };
...
export function chooseNextVisitDateObs(visitDateObs: Array<FHIRObsDatetime>) {
  return visitDateObs.reduce((previousObs: FHIRObsDatetime, currentObs: FHIRObsDatetime) => {
    if (!previousObs) {
      return currentObs;
    }
    ...
    if (previousDate <= new Date() && currentDate <= new Date() && currentDate >= previousDate) {
      return currentObs;
    }
    return previousObs;
  }, null);
